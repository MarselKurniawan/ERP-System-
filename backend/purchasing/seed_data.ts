  ...
  async () => {
    ...
    for (const supplier of suppliers) {
      ...
      if (!existingSupplier) {
        const newSupplier = await purchasingDB.queryRow<{ id: number }>`
          ...
          VALUES (${supplier.name}, ${supplier.email}, ${supplier.phone}, ${supplier.address}, ${supplier.taxId}, ${supplier.paymentTerms})
          RETURNING id
        `;
    ...
    const purchaseOrders = [
      {
        ...
        status: 'confirmed',
        items: [
          { productSku: 'LAPTOP001', productName: 'Business Laptop', quantity: 20, unitPrice: 12000000 },
          { productSku: 'MONITOR001', productName: '24-inch Monitor', quantity: 15, unitPrice: 2800000 }
        ],
        taxRate: 11,
      ...
      },
      {
        ...
        status: 'received',
        items: [
          { productSku: 'CHAIR001', productName: 'Office Chair', quantity: 25, unitPrice: 1800000 },
          { productSku: 'DESK001', productName: 'Office Desk', quantity: 20, unitPrice: 2200000 }
        ],
        taxRate: 11,
      ...
      },
      {
        ...
        status: 'sent',
        items: [
          { productSku: 'PAPER001', productName: 'A4 Paper', quantity: 100, unitPrice: 55000 }
        ],
        taxRate: 11,
      ...
      },
      {
        ...
        status: 'draft',
        items: [
          { productSku: 'MOUSE001', productName: 'Wireless Mouse', quantity: 50, unitPrice: 180000 }
        ],
        taxRate: 11,
        discountAmount: 0,
    ...
    for (const order of purchaseOrders) {
      ...
      const newOrder = await purchasingDB.queryRow<{ id: number }>`
        INSERT INTO purchase_orders (order_number, supplier_id, order_date, expected_date, status, subtotal, tax_amount, discount_amount, total_amount, notes)
        VALUES (${orderNumber}, ${order.supplierId}, ${order.orderDate}, ${order.expectedDate || null}, ${order.status}, ${subtotal}, ${taxAmount}, ${order.discountAmount || 0}, ${totalAmount}, ${order.notes || null})
        RETURNING id
      `;
