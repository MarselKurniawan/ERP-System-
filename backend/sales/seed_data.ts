  ...
  async () => {
    ...
    for (const customer of customers) {
      ...
      if (!existingCustomer) {
        const newCustomer = await salesDB.queryRow<{ id: number }>`
          ...
          VALUES (${customer.name}, ${customer.email}, ${customer.phone}, ${customer.address}, ${customer.taxId}, ${customer.creditLimit})
          RETURNING id
        `;
    ...
    const salesOrders = [
      {
        ...
        status: 'confirmed',
        items: [
          { productSku: 'LAPTOP001', productName: 'Business Laptop', quantity: 5, unitPrice: 15000000 },
          { productSku: 'MOUSE001', productName: 'Wireless Mouse', quantity: 5, unitPrice: 250000 }
        ],
        taxRate: 11,
      ...
      },
      {
        ...
        status: 'shipped',
        items: [
          { productSku: 'CHAIR001', productName: 'Office Chair', quantity: 10, unitPrice: 2500000 },
          { productSku: 'DESK001', productName: 'Office Desk', quantity: 10, unitPrice: 3000000 }
        ],
        taxRate: 11,
      ...
      },
      {
        ...
        status: 'draft',
        items: [
          { productSku: 'PAPER001', productName: 'A4 Paper', quantity: 50, unitPrice: 75000 },
          { productSku: 'PEN001', productName: 'Ballpoint Pen', quantity: 20, unitPrice: 25000 }
        ],
        taxRate: 11,
        discountAmount: 0,
    ...
    for (const order of salesOrders) {
      ...
      const newOrder = await salesDB.queryRow<{ id: number }>`
        INSERT INTO sales_orders (order_number, customer_id, order_date, due_date, status, subtotal, tax_amount, discount_amount, total_amount, notes)
        VALUES (${orderNumber}, ${order.customerId}, ${order.orderDate}, ${order.dueDate || null}, ${order.status}, ${subtotal}, ${taxAmount}, ${order.discountAmount || 0}, ${totalAmount}, ${order.notes || null})
        RETURNING id
      `;
